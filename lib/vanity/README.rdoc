...
== Using an ActiveRecord store

By default, Vanity uses Redis to store its data.  It is possible to use a ActiveRecord database store by adding a custom initializer: 

  Vanity.playground = Vanity::Playground.new(:adapter => :active_record)
